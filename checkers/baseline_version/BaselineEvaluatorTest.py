...
def one_lane_follow_test(test_index:int, duration: int, road_length: int):
    run_evaluator(test_index, [], ['z0', 'z1'], 
        ["@z0 !(Back 1)", #SV is initially at the start of the lane
         ...
         ["G(!(@z0 z1))"], (road_length,1), duration, False)
...
def safe_intersection_priority(test_index: int, duration: int, grid_size: int):
    run_evaluator(test_index, [], ['z0', 'z1'], 
                  ["@z1 !(Left 1)", #z1 starts somewhere on the left border
                   ...
                   ["G(!(@z0 z1))"], (grid_size, grid_size), duration, False)
...
def safe_passing(test_index: int, duration: int, road_length: int):
    ...
    run_evaluator(test_index, [], ['z0', 'z1'], 
                  ["G(@z1 !(Right 1))", # POV starts anywhere in right lane, stays in right lane
                   ...
                   ["G(!(@z0 z1))"], (road_length, 2), duration, False)
...
def join_platoon(test_index: int, duration: int, platoon_size: int, road_length: int):
    ...
    run_evaluator(test_index, [], noms, assumps, [postcond], (road_length, 2), duration, False)
